...
</li>`)}),t.innerHTML=n.join(``),s?s.refresh():s=new o.default.default(`.gallery a`,{captionsData:`alt`,captionDelay:250,download:!0})}function u(){let e=document.querySelector(`.gallery`);e&&(e.innerHTML=``,s&&=(s.destroy(),null))}function d(){c&&c.classList.remove(`display-none`)}function f(){c&&c.classList.add(`display-none`)}var p=document.querySelector(`.form`);p.addEventListener(`submit`,m);function m(e){e.preventDefault();let t=p.elements[`search-text`].value.trim();if(!t){i.default.show({message:`Please enter a search query.`,position:`topRight`,messageColor:`#fff`,backgroundColor:`red`});return}d(),u(),a(t).then(e=>{if(e.length===0){i.default.show({message:`Sorry, there are no images matching your search query. Please try again!`,position:`topRight`,messageColor:`#fff`,backgroundColor:`red`});return}l(e)}).catch(e=>{i.default.show({message:`Ooops, something went wrong, try again. ${e}`,position:`topRight`,messageColor:`#fff`,backgroundColor:`red`})}).finally(()=>{f()}),p.reset()}
